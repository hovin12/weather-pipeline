...
def weather_pipeline():
    ...
    def transfer_cities_task():
        unpack_and_transfer(source='raw_results', target='weather', pk=['run_ts', 'city'])
    ...
    def transfer_stations_task():
        unpack_and_transfer(source='raw_stations', target='weather_stations', pk=['run_ts', 'city', 'direction'])

    ingest_cities_task() >> transfer_cities_task() >> ingest_stations_task() >> transfer_stations_task()
